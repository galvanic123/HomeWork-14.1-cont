...
def test_category_add_product():                # type: ignore[no-untyped-def]
...
def test_category_all_products_count():          # type: ignore[no-untyped-def]
...
def test_custom_exception(capsys, category3):              # type: ignore[no-untyped-def]
    print(len(category3.product))
    assert len(category3.product) == 1


def test_add_product_invalid():          # type: ignore[no-untyped-def]
    category = Category("Телефизоры", "Техника для дома", "Smart TV")
    with pytest.raises(TypeError):
        category.add_product("Одежда")


def test_price():              # type: ignore[no-untyped-def]
